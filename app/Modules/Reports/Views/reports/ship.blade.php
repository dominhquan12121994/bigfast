                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="flex text-right">
                            @if( $filter['type'] != 'pickup')
                                <a href="{{ route('admin.report.by-ship') }}?type=pickup">Nhân viên lấy hàng</a>&nbsp;|&nbsp;
                            @else
                            Nhân viên lấy hàng&nbsp;|&nbsp;
                            @endif
                            @if( $filter['type'] != 'shipper')
                                <a href="{{ route('admin.report.by-ship') }}?type=shipper">Nhân viên ship</a>&nbsp;|&nbsp;
                            @else
                            Nhân viên ship&nbsp;|&nbsp;
                            @endif
                            @if( $filter['type'] != 'refund')
                                <a href="{{ route('admin.report.by-ship') }}?type=refund">Nhân viên chuyển hoàn</a>
                            @else
                            Nhân viên chuyển hoàn
                            @endif
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="row">
                                ...
                                <table id="table1" class="table table-responsive-sm table-responsive-md table-responsive-lg table-striped">
                                    <thead>
                                        <tr>
                                            ...
                                            @elseif( $filter['type'] == 'pickup')
                                                <th>Nhân viên lấy hàng</th>
                                                <th>Tổng đơn</th>
                                                <th>Lấy thành công</th>
                                                <th>Lấy không thành công</th>
                                            @else
                                                <th>Nhân viên chuyển hoàn</th>
                                                <th>Tổng đơn</th>
                                    ...
                                    <tbody>
                                        @foreach ($data as $key => $item)
                                            <tr>
                                                <td><a class="showDetail" href="javascript:void(0)" onclick="showDetail(`{{$key}}`,`{{ $item['name'] }}`, this)">{{ $item['name'] }}</a></td>
                                                <td>{{ $item['total'] }}</td>
                                                <td>{{ $item['success'] }}</td>
                                                <td>{{ $item['fail'] }}</td>
                                            </tr>
                            ...
                            <div class="spinner-border" role="status" style="margin:auto;display:none;">
...
@section('javascript')
    ...
    <script>
        ...
        let apiShowDetail = null;

        function showDetail (id, name, event) {
            let is_disbled = $(event).attr('disabled');
            if (typeof is_disbled !== 'undefined' && is_disbled !== false) {
                return;
            }

            $('.showDetail').attr('disabled', "disabled");
            $('.showDetail').addClass('color-dark');

            handelCallApi(id, name);
            
            setTimeout(function(){ 
                $('.showDetail').removeAttr('disabled');
                $('.showDetail').removeClass('color-dark');
            }, 1000);
        }

        function handelCallApi(id, name) {
            ...
            $.ajax({
                ...
                method: "POST",
                url: `{{ route('api.report.listShopByShip') }}`,
                timeout: 5000,
                data: {
                    'user_id': id,
                    'begin': `{{ $filter['created_range'][0] }}`,
                    'end': `{{ $filter['created_range'][1] }}`,
                    'type': `{{ $filter['type'] }}`,
                },
            })
            .done(function( msg ) {
                if ( msg.status_code === 200 ) {
                    if ( msg.data.result[0] ) {
                        let result = Object.values(msg.data.result[0]);
