...
@section('content')
                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
					<div class="card">
						<div class="card-header">
							<i class="fa fa-align-justify"></i>{{ __('Cấu hình mức phí lấy hàng') }}
							@if($currentUser->can('action_order_settings_pick_create'))
								...
								<div class="tab-content" id="nav-tabContent">
									<div class="tab-pane fade active show table-responsive" role="tabpanel" aria-labelledby="list-contacts-list">
										<table class="table table-striped" id="table1">
											<thead>
												<tr>
													<th>Số lượng đơn/ngày</th>
													<th>Phí/đơn</th>
													@if($currentUser->can('action_order_settings_pick_update') || $currentUser->can('action_order_settings_pick_delete'))
											...
											<tbody>
												@foreach( $cods as $key => $item )
													<tr class="data-id-{{$item->id}}">
														<td>Từ {{ number_format($item->min) }}{{ $item->max == 0 ? " đơn trở lên" : ' đến '. number_format($item->max) . ' đơn'  }}</td>
														<td>{{ number_format($item->value) }}</td>
														@if($currentUser->can('action_order_settings_pick_update') || $currentUser->can('action_order_settings_pick_delete'))
														<td>
															<form action="{{ route('admin.fee-pick.destroy', $item->id) }}" method="POST">
																@method('DELETE')
																<input type="hidden" name="_token" value="{{ csrf_token() }}" />
																@if($currentUser->can('action_order_settings_pick_update'))
																	<button type="button" class="btn btn-sm btn-primary" onclick='handleShowEdit({{$item->id}})'>Sửa</button>
																@endif
																@if($currentUser->can('action_order_settings_pick_delete'))
	...
	<div class="modal fade" id="crudModal" tabindex="-1" role="dialog" aria-labelledby="crudModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
		<div class="modal-dialog" role="document" style="margin-top:10%">
			...
			<div class="modal-header">
				<h5 class="modal-title">Thêm mới mức phí lấy hàng</h5>
			...
			<div class="modal-body row">
				<div class="col-md-12">
					<label for="sla">Số lượng đơn/ngày<b style="color:red"> (*)</b></label>
					<div class="input-group">
						...
						<div class="input-group-append"><span class="input-group-text">đơn</span></div>
					</div>
					<div class="input-group">
				...
				<div class="col-md-12" style="margin-top:10px">
					<label for="sla">Phí/đơn<b style="color:red"> (*)</b> </label>
					<div class="input-group">
	...
	<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title">Cập nhật mức phí lấy hàng</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">&times;</span>
				</button>
			</div>
...
@section('javascript')
	<script>
		let apiStore = `{{ route("api.store-order-fee-pick") }}`;
		let headerAuthorization = `Bearer {{ $currentUser->passport_token }}`;
		let routeIndex = `{{ route('admin.fee-pick.index') }}`;
		let apiFind = `{{ route("api.find-order-fee-pick") }}`;
		let apiUpdate = `{{ route("api.update-order-fee-pick") }}`;
	</script>

	<script src="{{ asset('js/pages/orders/feePick/list.min.js') }} "></script>
@endsection
