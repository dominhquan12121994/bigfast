...
@section('content')
                ...
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            @include('Orders::orders.shipper.mobile.count-status')

                            @include('Orders::orders.shipper.mobile.filter')

                            @include('Orders::orders.shipper.mobile.order-item')
    ...
    @include('Orders::orders.shipper.mobile.order-actions')
...
@section('javascript')
    ...
    <script src="{{ asset('libs/tinydatepickerv1/js/datepicker.min.js') }}"></script>
    <link href="{{ asset('libs/tinydatepickerv1/css/datepicker.min.css') }}" rel="stylesheet" type="text/css">
    <script src="{{ asset('libs/tinydatepickerv1/js/i18n/datepicker.en.js') }}"></script>
    ...
    <script type="application/javascript">
        ...
        jQuery(function($) {
            $('body').on('click', '#selectall', function() {
                ...
                if (this.checked) {
                    $('.singlechkboxBig').css('background-color', 'aliceblue');
                    window.scrollTo(0,document.body.scrollHeight);
                } else {
                    $('.singlechkboxBig').css('background-color', 'white');
                }
            });
            ...
            $('body').on('click', '.singlechkboxBig', function(e) {
                let box = $(this);
                let inputBox = $(box).find('.singlechkbox');
                let input = $(inputBox);
                let checked = input.is(':checked');
                ...
                if (checked) {
                    ...
                    $(box).css('background-color', 'aliceblue');
        ...
        let datePicker = $('.filter_datepicker').datepicker({
            range: true,
        ...
        datePicker.selectDate([ moment(`{{ $filter['created_range'][0] }}`).toDate(), moment(`{{ $filter['created_range'][1] }}`).toDate() ]);
        ...
        function handleDatePicker() {
            ...
            startDateFilter = moment(daterange[0]);
            endDateFilter = moment(daterange[0]);

            if ( daterange.length > 1 ) {
                endDateFilter = moment(daterange[1]);
            }
            HoldOn.open({theme:"sk-rect"});
            let route = '{{ route('admin.orders.index') }}';
            let newParams = '&begin=' + startDateFilter.format('DD-MM-YYYY') + '&end=' + endDateFilter.format('DD-MM-YYYY');
            window.location = reFormatUriParam(route, newParams);
        }

        @if ( (int)$orders->lastPage() != 1)
            function loadMoreOrderMobile () {
                if($(window).scrollTop() == $(document).height() - $(window).height() && !loadingMore) {
                    page++;
                    ...
                    $.ajax({
                        ...
                        url: `{{ route('api.shop.orders.load-more') }}`,
                        dataType: 'json',
                        type: 'post',
                        contentType: 'application/json',
                        data: JSON.stringify({
                            ...
                            "status_detail": `{{ $filter['status_detail'] }}`,
                            "limit" : `{{ $filter['limit'] }}`,
                            "created_range": `{{ implode(',', $filter['created_range']) }}`,
                        }),
                        processData: false,
                        success: function( data, textStatus, jQxhr ){
                            ...
                            data.data.html.forEach(function (item, index) {
