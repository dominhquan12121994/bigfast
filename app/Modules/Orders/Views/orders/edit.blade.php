    ...
    <div class="container-fluid">
        <div class="animated fadeIn">
            <div class="row">
                ...
                <div class="col-12">
                    <div class="card">
                        <form method="POST" id="frmOrderStore" action="{{ route('admin.orders.update', array('order' => $order->id)) }}">
                            <div class="card-header justify-content-between d-block d-sm-flex">
                                <div>
                                    <i class="fa fa-align-justify"></i> <strong>{{ __('Chỉnh sửa đơn hàng') }} - {{ $order->lading_code }} </strong>
                                </div>
                                <div>
                                    <button class="btn btn-success init-disable mt-2 mt-sm-0" type="submit">Cập nhật</button>
                                    <a href="{{ url()->previous() }}" class="btn btn-primary mt-2 mt-sm-0">Quay lại</a>
                                </div>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-md-12 col-lg-6">
                                        ...
                                        <h4>Bên gửi
                                            <span style="font-size: 12px;">(<a href="{{ route('admin.shops.edit', array('shop' => $shop->id)) . '?create-order' }}">
                                        ...
                                        @if($shopAddress)
                                            <div class="mb-1" style="line-height: 25px">
                                                ...
                                                <div class="btn-group" style="margin-top: -5px">
                                                    ...
                                                    <i data-toggle="tooltip" html="true" title="Thay đổi địa chỉ lấy hàng từ danh sách các địa chỉ đã đăng ký" class="fa fa-question-circle ml-1 text-danger"></i>
                                            ...
                                            <input type="hidden" name="senderId" id="senderId" value="{{ $shopAddress->id }}">
                                            ...
                                            <div class="showHide2">
                                                ...
                                                <input type="hidden" name="address_refund" id="address_refund" value="{{ $order->refund_id }}">
                                            </div>

                                            <div id="box_address_refund_selected" class="showHide2 mt-3 ml-3">
                                                <h6>Địa chỉ chuyển hoàn</h6>
                                                <p class="mb-1" id="txt_box_address_refund_selected">
                                                    {{ $order->refund->name }} - {{ $order->refund->phone }}<br>{{ $order->refund->address }}
                                                </p>
                                            </div>
                                        ...
                                        @else
                                            <p>
                                                ...
                                                <a href="{{ route('admin.shops.edit', array('shop' => $shop->id)) . '?create-order' }}">
                                    ...
                                    <div class="col-md-12 col-lg-6">
                                        ...
                                        <div class="form-group row mt-3">
                                            <div class="col-4">
                                                <input name="receiverName" class="form-control form-control-sm" type="text" value="{{ old('receiverName', $order->receiver->name) }}"
                                                       required placeholder="Tên người liên hệ">
                                            </div>
                                            <div class="col-4">
                                                <input name="receiverPhone" class="form-control form-control-sm" type="text" value="{{ old('receiverPhone', $order->receiver->phone) }}"
                                                       required placeholder="Số điện thoại">
                                            </div>
                                            <div class="col-4">
                                                <input name="receiverAddress" class="form-control form-control-sm" type="text" value="{{ old('receiverAddress', $order->receiver->address) }}"
                                            ...
                                            <div class="col-4">
                                                <select class="form-control frm-select2 change_to_calculator_fee" id="add1-select1" name="receiverProvinces" onchange="changeProvinces('add1')">
                                                    @foreach ($provinces as $province)
                                                        <option value="{{ $province->id }}" {{ ($order->receiver->p_id == $province->id) ? 'selected="selected"' : '' }}>{{ $province->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-4">
                                                <select class="form-control frm-select2 change_to_calculator_fee" id="add1-select2" name="receiverDistricts" onchange="changeDistricts('add1')">
                                                    @foreach ($districts as $district)
                                                        <option value="{{ $district->id }}" {{ ($order->receiver->d_id == $district->id) ? 'selected="selected"' : '' }}>{{ $district->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-4">
                                                <select class="form-control frm-select2" id="add1-select3" name="receiverWards">
                                                    @foreach ($wards as $ward)
                                                        <option value="{{ $ward->id }}" {{ ($order->receiver->w_id == $ward->id) ? 'selected="selected"' : '' }}>{{ $ward->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-lg-6">
                                                <div class="form-group row">
                                                    <div class="col">
                                                        <b style="font-size: 14px">Hàng hóa - <span id="lb_quantity_products">{{ $countProduct }}</span> sản phẩm (tên, giá trị, số lượng, mã)</b>
                                                        <span class="float-right">
                                                            <button class="btn btn-sm btn-block btn-outline-dark active init-disable" type="button" aria-pressed="true" onclick="plus()">
                                                                Thêm sản phẩm
                                                            </button>
                                                            <input type="hidden" id="quantity_products" name="quantity_products" value="{{ $countProduct }}">
                                                        </span>
                                                    </div>
                                                </div>
                                                <div id="product_arr">
                                                    @for ($i = 0; $i < $countProduct; $i++)
                                                        <div class="product_item form-group row">
                                                            <div class="col-4 pr-1">
                                                                <input name="addProductName[]" class="form-control init-disable" type="text" value="{{ $products[$i]->name }}" required placeholder="Tên sản phẩm">
                                                            </div>
                                                            <div class="col-3 pl-1 pr-1">
                                                                <div class="input-group">
                                                                    <input name="addProductPrice[]" class="form-control init-disable input_calculator_money" type="number" value="{{ $products[$i]->price }}" required placeholder="Giá trị sản phẩm" min="0" step="1000">
                                                                    <div class="input-group-append"><span class="input-group-text">vnđ</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col-2 pl-1 pr-1">
                                                                <div class="input-group">
                                                                    <input name="addProductSlg[]" class="form-control init-disable input_calculator_money" type="number" value="{{ $products[$i]->quantity }}" required placeholder="Số lượng" min="1" step="1">
                                                                    <div class="input-group-append"><span class="input-group-text">cái</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col-2 pl-1 pr-1">
                                                                <input name="addProductCode[]" class="form-control init-disable" type="text" value="{{ $products[$i]->code }}" placeholder="Nhập mã tự quản lý">
                                                            </div>
                                                            <div class="col-1 pl-1">
                                                                <input type="hidden" name="addProductId[]" value="{{ $products[$i]->id }}">
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-md-9 input-group">
                                                        <input class="form-control init-disable change_to_calculator_fee" type="number" name="weight" value="{{ old('weight', $order->weight) }}" step="1" min="1" required>
                                                    ...
                                                    <div class="col-md-9">
                                                        <div class="row">
                                                            <div class="col-4 input-group">
                                                                <input class="form-control init-disable" type="number" name="length" value="{{ old('length', $order->length) }}" step="1" min="0" required>
                                                                <div class="input-group-append"><span class="input-group-text">cm</span></div>
                                                            </div>
                                                            <div class="col-4 pl-0 pr-0 input-group">
                                                                <input class="form-control init-disable" type="number" name="width" value="{{ old('width', $order->width) }}" step="1" min="0" required>
                                                                <div class="input-group-append"><span class="input-group-text">cm</span></div>
                                                            </div>
                                                            <div class="col-4 input-group">
                                                                <input class="form-control init-disable" type="number" name="height" value="{{ old('height', $order->height) }}" step="1" min="0" required>
                                                                <div class="input-group-append"><span class="input-group-text">cm</span></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-md-9 input-group">
                                                        <input class="form-control init-disable" type="number" id="input_cod" name="cod" value="{{ old('cod', $order->cod) }}" step="1000" min="0" required>
                                                        <div class="input-group-append"><span class="input-group-text">vnđ</span></div>
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label class="col-md-3 col-form-label">Khai giá<i data-toggle="tooltip" html="true" title="Giá trị thực tế của đơn hàng" class="fa fa-question-circle ml-1 text-danger"></i></label>
                                                    <div class="col-md-9 input-group">
                                                        <input class="form-control init-disable" type="number" id="input_insurance_value" name="insurance_value" value="{{ old('insurance_value', $order->insurance_value) }}" step="1000" min="0" required>
                                                        <div class="input-group-append"><span class="input-group-text">vnđ</span></div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-12 col-lg-6">
                                                    ...
                                                    <div class="pl-4">
                                                        <div class="row">
                                                            @foreach($orderServices as $key => $service)
                                                                <div class="col-4 p-2">
                                                                    <input class="form-check-input init-disable" id="inline-radio{{ $key + 1 }}" type="radio" value="{{ $service->alias }}" name="service_type" {{ (old('service_type', $order->service_type) == $service->alias) ? 'checked="checked"' : '' }}>
                                                    ...
                                                    <div class="col-6">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control change_to_calculator_fee" name="expect_pick">
                                                                ...
                                                                <option value="{{ $key }}" {{ old('expect_pick', $order->extra->expect_pick) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" name="payfee">
                                                                @foreach($arrPayfee as $key => $value)
                                                                    <option value="{{ $key }}" {{ old('payfee', $order->payfee) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="form-group row">
                                                    <div class="col-6">
                                                        <div class="form-group">
                                                            <label>Lưu ý giao hàng<i data-toggle="tooltip" html="true" title="Giúp shipper biết có cho phép khách hàng xem hàng, dùng thử hay không" class="fa fa-question-circle ml-1 text-danger"></i></label>
                                                            <select class="form-control" name="note1">
                                                                @foreach($arrNote1 as $key => $value)
                                                                    <option value="{{ $key }}" {{ old('note1', $order->extra->note1) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="form-group">
                                                            <label>Mã đơn khách hàng tự quản</label>
                                                            <input class="form-control init-disable" type="text" name="client_code" value="{{ old('client_code', $order->extra->client_code) }}" placeholder="Nhập mã đơn khách hàng">
                                                        </div>
                                                    </div>
                                                    <div class="col-6">
                                                        <label>Ghi chú giao hàng</label>
                                                        <textarea class="form-control init-disable" name="note2" rows="5" placeholder="Ví dụ: Lấy sản phẩm A 2 cái">{{ old('client_code', $order->extra->note2) }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="card-footer">
                                <button class="btn btn-success init-disable" type="submit">Cập nhật</button>
