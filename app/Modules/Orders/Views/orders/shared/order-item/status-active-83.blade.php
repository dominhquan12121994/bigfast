<table class="table table-responsive table-striped" id="table_orders">
    ...
    <tr>
        ...
        <th class="text-right" style="min-width:170px">Tiền bồi hoàn(vnđ)<i data-toggle="tooltip" html="true" title="Tiền BigFast trả lại Shop khi xảy ra hư hỏng, thất lạc hàng hoá" class="fa fa-question-circle ml-1 text-danger"></i></th>
        <th width="160px"></th>
    </tr>
    </thead>
    <tbody>
        @foreach($orders as $key => $order)
            ...
            <tr>
                ...
                <td class="text-right">@include('Orders::orders.shared.money.other_fee', compact('order', 'incurred_fee'))</td>
                <td class="text-right">@include('Orders::orders.shared.money.cod', compact('order', 'incurred_fee'))</td>
                <td class="text-right"> <span class="badge-light">{{ $incurred_money_indemnify }} </span></td>
                <td class="box-actions">
                    @include('Orders::orders.shared.button-actions')
                </td>
            </tr>
        @endforeach
    </tbody>
