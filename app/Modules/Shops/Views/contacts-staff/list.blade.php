            ...
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
					<div class="card">
						...
						<div class="card-body">
							<form class="form-inline" action=" {{ route('shop.contacts-staff.index') }}" method="GET">
								...
								<tbody>
									...
									<tr>
										...
										<td class="box-actions">
											<form action="{{ route('shop.contacts-staff.destroy', $value->id) }}" method="POST">
												@method('DELETE')
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<form method="POST" action="{{ route('shop.contacts-staff.store') }}" enctype="multipart/form-data">
					...
					<input type="hidden" name="_token" value="{{ csrf_token() }}" />
					<input type="hidden" name="route" value="shop.contacts-staff.index">
...
@section('javascript')
	...
	<script>
		...
		let urlDownloadContact = `{{ url('/contacts-staff/download') }}`;
		let shopId = `{{ $shop->id }}`;
		if ( '{{ $errors->any() }}' == '1' ) {
			checkLading = true;
			$('.information').show();
