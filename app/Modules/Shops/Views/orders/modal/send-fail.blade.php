<form action="" method="post" id="frm_send_fail">
    <div class="form-group row mb-1">
        <label class="col-md-3 col-form-label" for="fail_reason">Lý do</label>
        <div class="col-md-9">
            <select class="form-control" id="fail_reason" name="fail_reason">
                <option value="reason">Lý do không giao được hàng??</option>
            </select>
        </div>
    </div>
    <div class="form-group row mb-1">
        <label class="col-md-3 col-form-label" for="fail_note">Mô tả</label>
        <div class="col-md-9">
            <textarea class="form-control" id="fail_note" name="fail_note" rows="3" placeholder="Mô tả chi tiết" required></textarea>
        </div>
    </div>
    <div class="form-group row justify-content-end mb-0">
        <button class="btn btn-info" type="submit">Xác nhận</button>
        <button class="btn btn-secondary ml-2" type="button" data-dismiss="modal">Huỷ</button>
    </div>
</form>